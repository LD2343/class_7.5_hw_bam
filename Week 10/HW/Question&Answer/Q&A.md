...
-  Load balancers use the target proxy to decrypt incoming encrypted traffic.
### Are there use cases to have in flight encryption from the backend service to the backend itself?
- Yes there are use cases but it is usually for PII & Healthcare.

## Cloud Domain/DNS

### Can multiple domains end up pointing to the same LB?
-  Yes this can be done for multi-tenant applications to point to the same load balancer.
### In the context of Cloud DNS, what are zones?
- There are 2 types of zones public and private. The public zones are accessible via the internet(like webistes & load balancers) and the private zones are for internal services (like databases).