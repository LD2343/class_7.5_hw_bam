...
- Vertical Autoscaling is scaling up in terms of increasing a server's capacity (increasing CPU/Memory/Storage).
- Horizontal Autoscaling is scaling out in terms of adding more servers to share the workload (adding more VMs to the MIG).
...
- Unmanaged Instance Groups: This is for user-managing load balancing across non-identical VMs. Doesn't include existing automated configuration for autoscaling or autohealing.
...
- Technically they can be the same but it is recommended they are different since the health check for a load balancer should be more aggressive because these checks route user traffic. The health checks for MIGs should be less aggressive since they are checking if autohealing is needed. Both have the same API calls.
...
- 3 Tier architecture is made up of 3 design levels:
    - Web Tier - Application frontend that usually receives traffic from an external ALB to send the traffic to the frontend MIGs which will then send to the internal ALB.
...
- The goal of this runbook is to provide instruction on how to create a managed instance group (MIG) in GCP.
...
![](runbook%20screenshots/Pasted%20image%2020260504191614.png)
...
![](runbook%20screenshots/Pasted%20image%2020260504191726.png)
...
![](runbook%20screenshots/Pasted%20image%2020260504192054.png)
...
![](runbook%20screenshots/Pasted%20image%2020260507211122.png)
...
![](runbook%20screenshots/Pasted%20image%2020260507213529.png)
...
![](runbook%20screenshots/Pasted%20image%2020260507221327.png)

## Terraform VM Breakdown

### Required arguments for VM instance (google_compute_instance)
...
- zone
    - This argument we can use to place the vm in a specific zone.
- tags
    - This argument will allow attaching network tags to the instance

### VM Images

- The documentation lists the images - https://docs.cloud.google.com/compute/docs/images/os-details
